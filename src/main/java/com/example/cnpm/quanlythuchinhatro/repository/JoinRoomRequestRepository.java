...
public interface JoinRoomRequestRepository extends JpaRepository<JoinRoomRequest, Long> {
    ...
    @Query("SELECT j.requestDate AS requestDate, j.userId AS userId, u.name AS fullname, u.avatarUrl AS avatarUrl FROM JoinRoomRequest j JOIN User u ON j.userId = u.id WHERE j.roomId =:roomId AND j.status = 1")
